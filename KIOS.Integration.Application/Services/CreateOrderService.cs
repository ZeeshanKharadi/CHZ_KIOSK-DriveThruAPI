...
using POS_Integration_CommonCore.Model.Abstraction;
...
namespace KIOS.Integration.Application.Services
{
    public class CreateOrderService : ICreateOrderService
    {
        ...
        private string StaffId(string terminal)
        {
            ...
            DataSet ds = SqlHelper.ExecuteDataSet(_connectionString_CHZ_MIDDLEWARE, queryStaffId, CommandType.Text, null);
        ...
        private int InsertKDSOrders(KIOS.Integration.Application.Commands.CreateRetailTransactionCommand request)
        {
            int affectedRows = 0;

            string KioskOrderDetail = "usp_GetOrderDetail";
            Dictionary<string, object> KIOSKOrderDetailParameters = new Dictionary<string, object>
             {
                  { "ThirdPartyOrder", request.ThirdPartyOrderId}
             };
            ...
            {
                string InsertSimplexRequestLogQuery = "usp_InsertOrders";
                ...
                Dictionary<string, object> parameters = new Dictionary<string, object>
              {
                  ...
                  { "OrderNo", _receiptId },
                  { "CreatedOn", DateTime.Now },
                  { "OrderTypeID", dtRow["ConfigID"].ToString()},
                  { "OrderType", dtRow["ConfigIDName"].ToString()},
                  { "ItemCategory", dtRow["CATEGORY"].ToString() },
                  { "ItemID", dtRow["ItemId"].ToString() },
                  { "ItemName", dtRow["Item Name"].ToString() },
                  { "Quantity", Convert.ToDecimal(dtRow["QTY"].ToString()) * -1},
                  { "Description", dtRow["ITEM DESCRIPTION"].ToString() },
                  { "OrderStatusID", "1"},
                  { "OrderStatus", "Preparation" },
                  { "OrderState", "Preparing" },
                  { "LINENUM", Convert.ToDecimal(dtRow["LINENUM"].ToString())},
                  { "TransactionType", 2 },
                  { "OrderSource", "KIOSK order directly from Api thirdpartyOrderId:" + request.ThirdPartyOrderId }

              };

                affectedRows = SqlHelper.ExecuteNonQuery(_connectionString_KDS, InsertSimplexRequestLogQuery, CommandType.StoredProcedure, parameters);
            }
